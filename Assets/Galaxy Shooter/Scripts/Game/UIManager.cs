...
public class UIManager : MonoBehaviour  {
    ...
    private void Update() {
        if (_txtRestart.gameObject.activeSelf && Input.GetKeyDown(KeyCode.R)) SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
        else if (_txtRestart.gameObject.activeSelf && Input.GetKeyDown(KeyCode.Escape)) SceneManager.LoadScene(0);
    ...
    public void ChangeLivesDisplayImage(int noImage) {
        if (noImage < 0) noImage = 0;
        _livesDisplayImage.sprite = _liveSprites[noImage];
        if (noImage == 0)  GameOverSequence();
    }
